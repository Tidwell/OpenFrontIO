...
func GenerateMap(args GeneratorArgs) (MapResult, error) {
	...
	removeSmallIslands(terrain, args.RemoveSmall, args.Verbose)
	processWater(terrain, args.RemoveSmall, args.Verbose)

	terrain4x := createMiniMap(terrain)
	processWater(terrain4x, false, args.Verbose)

	terrain16x := createMiniMap(terrain4x)
	processWater(terrain16x, false, args.Verbose)
...
func processWater(terrain [][]Terrain, removeSmall bool, verbose bool) {
		...
		if removeSmall {
			...
			for w := 1; w < len(waterBodies); w++ {
				if waterBodies[w].size < minLakeSize {
					smallLakes++
					// Log only the starting pixel and size when verbose
					if verbose && len(waterBodies[w].coords) > 0 {
						start := waterBodies[w].coords[0]
						log.Printf("Removing lake of size %d starting at (%d,%d)", waterBodies[w].size, start.X, start.Y)
					}
...
func removeSmallIslands(terrain [][]Terrain, removeSmall bool, verbose bool) {
	if !removeSmall {
		return
	}
	...
	for _, body := range landBodies {
		if body.size < minIslandSize {
			smallIslands++
			// Log only the starting pixel and size when verbose
			if verbose && len(body.coords) > 0 {
				start := body.coords[0]
				log.Printf("Removing island of size %d starting at (%d,%d)", body.size, start.X, start.Y)
			}
